...
## 问题 N：审计日志 / 限速里的客户端 IP 不对（显示成 127.0.0.1 或代理 IP）

出于安全，hub 只在请求的 TCP 来源属于「可信代理」时，才采信 `X-Forwarded-For` / `X-Real-IP` 转发头——否则直连的攻击者可伪造该头绕过按 IP 的限速与封禁。

- **默认可信** = 本机回环 `127.0.0.1/32`、`::1/128`，覆盖标准的 nginx → hub（同机回环）部署，无需配置。
- **在 nginx 前还有一层代理 / CDN**（如 Cloudflare）时，hub 看到的来源是那层代理，需要把它的出口网段加入可信列表，否则真实客户端 IP 取不到：

```bash
# 逗号分隔的 CIDR;留空则完全不信任转发头(只用直连 IP)
SHIGUANG_TRUSTED_PROXIES="127.0.0.1/32,::1/128,10.0.0.0/8"
```

> 同时确保最外层代理会**重写**（而非透传客户端伪造的）`X-Forwarded-For`。
